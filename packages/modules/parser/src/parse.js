...
import { join, last, push, some, split } from 'uty/collect';
import { RAW_EMPTY, RGX_WHITESPACE } from 'uty/define';
import { isArray, isEmpty, isEqual, isUndefined } from 'uty/is';
import { size } from 'uty/math';
import { words } from 'uty/string';
import { DocumentNode, TextNode } from './ast.js';
import { RGX_HTML_OUTSIDE_TAGS, commentEnd, commentStart, textSelector } from './constant.js';
import { parseHtmlTag } from './tag.js';
import { parseOptions } from './utils.js';

/**
 * Parses the given program and returns the abstract syntax tree (AST).
 * @param {string} program The program to parse.
 * @returns {Array} The abstract syntax tree representing the structure.
 */
export function parse(program, options = {}) {
